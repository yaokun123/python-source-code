 ...
 */
typedef struct _object {        //// PyObject是python中不包含可变长度数据的对象的基石
    _PyObject_HEAD_EXTRA
    Py_ssize_t ob_refcnt;       // long ->  引用计数，类型对象是超越引用计数的规则的
    PyTypeObject *ob_type;      // 用来指定一个对象类型的类型对象
} PyObject;
...
typedef struct {                //// 包含可变长度数据的对象的对象的基石，像这样的对象通常都是容器
    PyObject ob_base;
    Py_ssize_t ob_size;         // 指明了该对象中一共包含了多少个元素（个数），比如list就是一个PyVarObject对象
} PyVarObject;
