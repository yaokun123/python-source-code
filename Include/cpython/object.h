...
// as well.
struct _typeobject {                            //// 类型对象
    PyObject_VAR_HEAD                           // PyVarObject ob_base
    const char *tp_name;                        // 类型名，主要是python内部以及调试的时候使用
    Py_ssize_t tp_basicsize, tp_itemsize;       // long ->  创建该类型对象是分配内存的大小的信息

    /* Methods to implement standard operations */

    destructor tp_dealloc;                      // 引用计数为0时调用，释放该对象所占有的内存和系统资源（函数指针）
                                                // 并不意味着最终一定会调用free释放释放内存空间，如果真是这样的话，
                                                // 那么频繁的申请和释放空间会使python的执行效率大打折扣，
                                                // 一般来说python中大量采用了内存对象池的技术
    ...
    hashfunc tp_hash;                           // 指明对于该类型的对象，如何生成其hash值（函数指针）
    ternaryfunc tp_call;
    reprfunc tp_str;
